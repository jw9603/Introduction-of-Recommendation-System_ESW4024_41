...
class CORE(torch.nn.Module):
    def __init__(self, num_items, hidden_dim, maxlen, num_blocks, num_heads, lr, batch_size, temperature, device):
        super().__init__()
    ...
    def build_graph(self):
        ...
        # Learnable weights
        self.fn = nn.Linear(self.hidden_dim, 1)
        self.fn.weight.data.normal_(mean=0.0, std=0.01)
    ...
    def Transformer(self, item_emb, inputs):
        # NOTE: Copy and paste your SASRec code
        
        # Item Embedding + Positional Embedding
        # ========================= EDIT HERE ========================
        seqs = self.item_emb(inputs)
        positions = torch.arange(inputs.shape[1], device=self.device).unsqueeze(0).repeat(inputs.size(0),1)
        ...
        return hidden_feats, timeline_mask
    
    def RCE(self, dnn_hidden_feats, item_emb, timeline_mask):
        # ========================= EDIT HERE ========================  
        # wF^T
        wF = self.fn(dnn_hidden_feats).squeeze(-1)
        # remove effects of zero padded sequences
        wF = wF.masked_fill(~timeline_mask, float('-inf'))
        # softmax
        alpha = F.softmax(wF/self.temperature, dim=-1)
        hidden_feats = torch.bmm(alpha.unsqueeze(1), dnn_hidden_feats).squeeze(1)
        # ========================= EDIT HERE ========================
        hidden_feats = F.normalize(hidden_feats, dim=-1)
        
        return hidden_feats

    def RDM(self, hidden_feats, item_embs):
        logits = None
        item_embs = F.normalize(item_embs, dim=-1)
        # ========================= EDIT HERE ========================
        logits = hidden_feats @ item_embs / self.temperature
        # ========================= EDIT HERE ========================  
        return logits
        
    def forward(self, inputs):
        # inputs: (batch_size, maxlen)
        item_emb = self.item_emb(inputs)
        
        # Transforemr
        dnn_hidden_feats, timeline_mask = self.Transformer(item_emb, inputs)
        
        # Representation-Consistent Encoder (RCE)
        hidden_feats = self.RCE(dnn_hidden_feats, item_emb, timeline_mask)
    ...
    def train_model_per_batch(self, inputs, targets):
        ...
        hidden_feats = self.forward(inputs)
        item_embs = self.emb_dropout(self.item_emb.weight[1:].T)
        
        # Robust Distance Measuring (RDM)
        logits = self.RDM(hidden_feats, item_embs)
...
class CORE_session:
    def __init__(self, hidden_dim=100, batch_size=128, valid_size=0.1, lr=0.001, num_epochs=500, num_blocks=2, num_heads=1, temperature=0.05,
                maxlen=50, step=1, patience=5, device='cuda', session_key='SessionId', item_key='ItemId', time_key='Time'):
        self.hidden_dim = hidden_dim
        self.batch_size = batch_size
    ...
    def fit(self, data, test=None, items=None):
        ...
        self.model = CORE(self.num_items, self.hidden_dim, self.maxlen, self.num_blocks, self.num_heads, self.lr,
                            self.batch_size, self.temperature, self.device).to(self.device)
        ...
        for epoch in range(self.num_epochs):
            ...
            if epoch % self.step == 0:
                ...
                print('Cur Result:')
                print('\tRecall@20:\t%.4f\tMRR@20:\t%.4f\tEpoch:\t%d' % (hit, mrr, epoch))
                print('Best Result:')
                print('\tRecall@20:\t%.4f\tMRR@20:\t%.4f\tEpoch:\t%d' % (best_result[0], best_result[1], best_epoch))
    ...
    def train_test(self, train_loader, test_loader, train=True):
        ...
        for i, (seqs, targets, lens) in tqdm(enumerate(test_loader), total=len(test_loader), desc='test', dynamic_ncols=True):
            ...
            hidden_feats = self.model.forward(seqs)
            item_embs = self.model.item_emb.weight[1:].T
            
            # Robust Distance Measuring (RDM)
            scores = self.model.RDM(hidden_feats, item_embs)
    ...
    def predict_next(self, session_id, input_item_id, predict_for_item_ids, input_user_id=None, timestamp=0, skip=False, type='view'):
        ...
        with torch.no_grad():
            ...
            hidden_feats = self.model.forward(seq)
            item_embs = self.model.item_emb.weight[1:].T
