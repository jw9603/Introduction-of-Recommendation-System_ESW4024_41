...
class SRGNN(nn.Module):
    ...
    def compute_scores(self, hidden, mask):
        ...
        # final scoring
        a = self.linear_W(torch.cat([a, ht], 1))
...
class SRGNN_session:
    def __init__(self, hidden_size=32, batch_size=128, nonhybrid=False, valid_size=0.1,
                lr=0.01, lr_dc=0.1, lr_dc_step=3, epoch=100, step=1, patience=5, l2=1e-5, device='cuda',
    ...
    def fit(self, data, test=None, items=None):
        ...
        self.itemidmap = pd.Series(data=np.arange(1, self.n_items+1), index=itemids)
        ...
        n_node = self.n_items+1
        max_pos = max(max([len(x) for x in train_data.inputs]), max([len(x) for x in valid_data.inputs])) + 1

        self.model = trans_to_cuda(SRGNN(self.hidden_size, self.batch_size, self.nonhybrid, self.step, self.lr,
